  ...
  <body>
	...
	<%
		...
		for (int f = 0; f < 1; /*f++*/) {
			...
			sql = "select * from tb_class where className = ?";		
			rs = db.select(sql, className);
			...
			sql = "insert into tb_class (className, year) values (?, ?)";
			String classId = db.insert(sql, className, year);
