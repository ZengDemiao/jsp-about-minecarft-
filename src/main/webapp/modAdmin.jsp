...
<head>
    ...
    <style>
        * {
        ...
        .mods-header {
            display: grid;
            grid-template-columns: 60px 1fr 1.5fr 1fr 1fr 1fr 1.5fr 100px 80px;
            padding: 16px 25px;
            background: var(--bg-secondary);
...
<body>
    <div class="container">
        <%
            ...
            if (request.getParameter("buttonDelete") != null) {                     //如果单击了删除按钮
                String[] modIdList = request.getParameterValues("modId");     //modId值数组
                
                if (modIdList != null) {
                    String modIdAll = "";
                    
                    for (int i = 0; i < modIdList.length; i++) {
                        try {                                      
                            Integer.parseInt(modIdList[i]);            //尝试转换为整数，即判断其是否为整数
                        } catch (Exception e) {
                            continue;                                    //如果转换失败，即不是整数，则略过此项
                        }
                        
                        modIdAll += "," + modIdList[i];    
                    }
                    
                    if (modIdAll.isEmpty() == false) {                //如果有值
                        modIdAll = modIdAll.substring(1);        //去除最前面的逗号
                        
                        Db db2 = new Db();                                //新建实例（该实例实现了数据的增删改查）                    
                        String sql2 = "delete from tb_mod where modId in (" + modIdAll + ")";
        ...
        <div class="nav-links">
            ...
            <a href="main.jsp"><i class="fas fa-home"></i> 用户功能</a>
            <a href="modList.jsp"><i class="fas fa-list"></i> 模组列表</a>
            <a href="modPage.jsp"><i class="fas fa-file-alt"></i> 模组分页</a>
            <a href="modAdmin.jsp" class="active"><i class="fas fa-cog"></i> 模组管理</a>
            <a href="userAdmin.jsp"><i class="fas fa-users-cog"></i> 用户管理</a>
        </div>
        ...
        <form action="" method="post">
            <div class="admin-tools">
                ...
                <a href="modAdd.jsp" class="add-mod-btn">
                    <i class="fas fa-plus"></i> 添加新模组
                </a>
            </div>
            ...
            <div class="mods-container">
                ...
                <%
                    ...
                    while (rs != null && rs.next()) {                        //如果读到了数据                                    
                        ...
                        String modId = rs.getString("modId");        //链接中要用到
                        
                        // 为服务器类型添加样式
                        String serverClass = "server-tag ";
                        if (server != null && server.equals("网易")) {
                            serverClass += "netease-server";
                        } else {
                            serverClass += "pc-server";
                        }
                %>
                <div class="mod-item">
                    ...
                    <div class="mod-server">
                        <span class="<%= serverClass %>"><%= server %></span>
                    </div>
                    <div class="mod-year"><%= year %></div>
                    <div class="mod-version"><%= version %></div>
                    <div class="mod-action action-cell">
                        <a href="modShow.jsp?modId=<%= modId %>" class="action-btn">
                            <i class="fas fa-eye"></i> 详情
                        </a>
                        <a href="modEdit.jsp?modId=<%= modId %>" class="action-btn edit-btn">
                            <i class="fas fa-edit"></i> 编辑
                        </a>
                    </div>
                    <div class="mod-checkbox checkbox-container">
                        <label class="custom-checkbox">
                            <input type="checkbox" name="modId" value="<%= modId %>">
            ...
            <div class="delete-section">
                ...
                <button type="submit" class="delete-btn" name="buttonDelete" 
                    onclick="return confirm('确认删除所选模组？该操作不可恢复！')">
                    <i class="fas fa-trash-alt"></i> 删除选中模组
                </button>
            </div>
            ...
            <div class="pagination">
                <div class="pagination-controls">
                    ...
                    <% } else { %>
                        <a href="modAdmin.jsp?pageUrl=1&searchUrl=<%= searchUrl %>" class="pagination-btn">
                            <i class="fas fa-angle-double-left"></i> 首页
                        </a>
                        <a href="modAdmin.jsp?pageUrl=<%= pageShow - 1 %>&searchUrl=<%= searchUrl %>" class="pagination-btn">
                            <i class="fas fa-angle-left"></i> 上一页
                        </a>
                    <% } %>
                    ...
                    <% } else { %>
                        ...
                        <a href="modAdmin.jsp?pageUrl=<%= pageCount %>&searchUrl=<%= searchUrl %>" class="pagination-btn">
                            尾页 <i class="fas fa-angle-double-right"></i>
                        </a>
                    <% } %>
                </div>
    ...
    function checkAll() {
        var checkboxList = document.getElementsByName("modId");            //获取复选框列表
